...
thf(conj_thm_2Erich__list_2EFOLDR__MAP,lemma,(
     ...
     => ! [V1e: $i] :
         ...
         => ! [V2g: $i] :
             ...
             => ! [V3l: $i] :
                 ...
                 => ( ( ap @ ( ap @ ( ap @ ( c_2Elist_2EFOLDR @ A_27a @ A_27b ) @ V0f ) @ V1e ) @ ( ap @ ( ap @ ( c_2Elist_2EMAP @ A_27c @ A_27a ) @ V2g ) @ V3l ) )
                    = ( ap
                      @ ( ap
                        @ ( ap @ ( c_2Elist_2EFOLDR @ A_27c @ A_27b )
                          @ ( lam @ A_27c
                            @ ^ [V4x: $i] :
                                ( lam @ A_27b
                                @ ^ [V5y: $i] :
                                    ( ap @ ( ap @ V0f @ ( ap @ V2g @ V4x ) ) @ V5y ) ) ) )
                        @ V1e )
                      @ V3l ) ) ) ) ) ) )).

thf(conj_thm_2Erich__list_2EMEM__FOLDR,lemma,(
    ! [A_27a: del,V0y: $i] :
      ( ( mem @ V0y @ A_27a )
     => ! [V1l: $i] :
         ...
         => ( ( p @ ( ap @ ( ap @ ( c_2Ebool_2EIN @ A_27a ) @ V0y ) @ ( ap @ ( c_2Elist_2ELIST__TO__SET @ A_27a ) @ V1l ) ) )
          <=> ( p
              @ ( ap
                @ ( ap
                  @ ( ap @ ( c_2Elist_2EFOLDR @ A_27a @ bool )
                    @ ( lam @ A_27a
                      @ ^ [V2x: $i] :
                          ( lam @ bool
                          @ ^ [V3l_27: $i] :
                              ( ap @ ( ap @ c_2Ebool_2E_5C_2F @ ( ap @ ( ap @ ( c_2Emin_2E_3D @ A_27a ) @ V0y ) @ V2x ) ) @ V3l_27 ) ) ) )
                  @ ( inj__o @ fo__c_2Ebool_2EF ) )
                @ V1l ) ) ) ) ) )).

thf(conj_thm_2Erich__list_2EMEM__FOLDR__MAP,conjecture,(
    ! [A_27a: del,V0x: $i] :
      ( ( mem @ V0x @ A_27a )
     => ! [V1l: $i] :
          ( ( mem @ V1l @ ( ty_2Elist_2Elist @ A_27a ) )
