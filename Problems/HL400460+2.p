include('Axioms/HL4001+2.ax').
fof(mem_c_2Emin_2E_3D,axiom,(
    ! [A_27a] :
      ( ne(A_27a)
     => mem(c_2Emin_2E_3D(A_27a),arr(A_27a,arr(A_27a,bool))) ) )).

fof(ax_eq_p,axiom,(
    ! [A] :
      ( ne(A)
     => ! [X] :
          ( mem(X,A)
         => ! [Y] :
              ( mem(Y,A)
             => ( p(ap(ap(c_2Emin_2E_3D(A),X),Y))
              <=> X = Y ) ) ) ) )).

fof(mem_c_2Emin_2E_3D_3D_3E,axiom,(
    mem(c_2Emin_2E_3D_3D_3E,arr(bool,arr(bool,bool))) )).

fof(ax_imp_p,axiom,(
    ! [Q] :
      ( mem(Q,bool)
     => ! [R] :
          ( mem(R,bool)
         => ( p(ap(ap(c_2Emin_2E_3D_3D_3E,Q),R))
          <=> ( p(Q)
             => p(R) ) ) ) ) )).

fof(mem_c_2Ebool_2E_21,axiom,(
    ! [A_27a] :
      ( ne(A_27a)
     => mem(c_2Ebool_2E_21(A_27a),arr(arr(A_27a,bool),bool)) ) )).

fof(ax_all_p,axiom,(
    ! [A] :
      ( ne(A)
     => ! [Q] :
          ( mem(Q,arr(A,bool))
         => ( p(ap(c_2Ebool_2E_21(A),Q))
          <=> ! [X] :
                ( mem(X,A)
               => p(ap(Q,X)) ) ) ) ) )).

fof(mem_c_2Ebool_2E_2F_5C,axiom,(
    mem(c_2Ebool_2E_2F_5C,arr(bool,arr(bool,bool))) )).

fof(ax_and_p,axiom,(
    ! [Q] :
      ( mem(Q,bool)
     => ! [R] :
          ( mem(R,bool)
         => ( p(ap(ap(c_2Ebool_2E_2F_5C,Q),R))
          <=> ( p(Q)
              & p(R) ) ) ) ) )).

fof(mem_c_2Erelation_2ERSUBSET,axiom,(
    ! [A_27a] :
      ( ne(A_27a)
     => ! [A_27b] :
          ( ne(A_27b)
         => mem(c_2Erelation_2ERSUBSET(A_27a,A_27a),arr(arr(A_27a,arr(A_27b,bool)),arr(arr(A_27a,arr(A_27b,bool)),bool))) ) ) )).

fof(mem_c_2Erelation_2EWeakOrder,axiom,(
    ! [A_27g] :
      ( ne(A_27g)
     => mem(c_2Erelation_2EWeakOrder(A_27g),arr(arr(A_27g,arr(A_27g,bool)),bool)) ) )).

fof(conj_thm_2Erelation_2EWeakOrder__EQ,lemma,(
    ! [A_27a] :
      ( ne(A_27a)
     => ! [V0R] :
          ( mem(V0R,arr(A_27a,arr(A_27a,bool)))
         => ( p(ap(c_2Erelation_2EWeakOrder(A_27a),V0R))
           => ! [V1y] :
                ( mem(V1y,A_27a)
               => ! [V2z] :
                    ( mem(V2z,A_27a)
                   => ( V1y = V2z
                    <=> ( p(ap(ap(V0R,V1y),V2z))
                        & p(ap(ap(V0R,V2z),V1y)) ) ) ) ) ) ) ) )).

fof(conj_thm_2Erelation_2ERSUBSET__WeakOrder,lemma,(
    ! [A_27a] :
      ( ne(A_27a)
     => ! [A_27b] :
          ( ne(A_27b)
         => p(ap(c_2Erelation_2EWeakOrder(arr(A_27a,arr(A_27b,bool))),c_2Erelation_2ERSUBSET(A_27a,A_27a))) ) ) )).

fof(conj_thm_2Erelation_2EEqIsBothRSUBSET,conjecture,(
    ! [A_27a] :
      ( ne(A_27a)
     => ! [A_27b] :
          ( ne(A_27b)
         => ! [V0y] :
              ( mem(V0y,arr(A_27a,arr(A_27b,bool)))
             => ! [V1z] :
                  ( mem(V1z,arr(A_27a,arr(A_27b,bool)))
                 => ( V0y = V1z
                  <=> ( p(ap(ap(c_2Erelation_2ERSUBSET(A_27a,A_27a),V0y),V1z))
                      & p(ap(ap(c_2Erelation_2ERSUBSET(A_27a,A_27a),V1z),V0y)) ) ) ) ) ) ) )).
